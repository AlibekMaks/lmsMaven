...
public class ADLSeqUtilities {
    ...
    public static void createGlobalObjs(String iLearnerID,
                                        String iScopeID,
                                        Vector iObjList, Statement st) throws Exception {
        ...
        if (iLearnerID != null) {
            if (iObjList != null) {
                for (int i = 0; i < iObjList.size(); i++) {
                    ...
                    res = st.executeQuery("SELECT COUNT(*) FROM objectives WHERE " +
                            " objID = '" + trsf.getDBString(objID) + "' AND  " +
                            " learnerID = '" + trsf.getDBString(iLearnerID) + "' " +
                            " AND scopeID = '" + trsf.getDBString(iScopeID) + "' ");
                    ...
                    if (!res.next() || res.getInt(1) == 0) {
                        ...
                        st.execute("INSERT INTO objectives " +
                                "(objID, learnerID, scopeID, satisfied, measure) " +
                                " VALUES ('" + trsf.getDBString(objID) + "', " +
                                " '" + trsf.getDBString(iLearnerID) + "', " +
                                " '" + trsf.getDBString(iScopeID) + "' , " +
                                " 'unknown', 'unknown')");
