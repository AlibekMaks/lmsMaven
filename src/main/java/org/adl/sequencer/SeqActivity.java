...
SeqActivityStateAccess, Serializable
{
   ...
   */
   ...
   private void setParent(SeqActivity iParent)
   {
    ...
    public void deleteBadBytes(){
        updateTitle();
        if (mChildren != null && mChildren.size() > 0){
            for (int i  = 0; i  < mChildren.size(); i ++){
                if (mChildren.get(i) instanceof SeqActivity){
                    ((SeqActivity)mChildren.get(i)).deleteBadBytes();
                }
            }
        }
    }
    ...
    public void updateTitle(){
        try{
            byte [] b = mTitle.getBytes();
            for (int i = 0; i < b.length - 1; i ++){
                if (b[i] == -45 && b[i + 1] == 63){
                    b[i + 1] = -104;
                } else if (b[i] == -48 && b[i+1] == 63){
                    b[i + 1] = -104;
                }
            }
            mTitle = new String(b, "UTF-8");
        } catch(Exception exc){
            Log.writeLog(exc);
        }
    }
