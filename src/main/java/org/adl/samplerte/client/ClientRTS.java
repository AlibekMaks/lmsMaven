...
public class ClientRTS extends Applet implements SCORM13APIInterface {
    ...
    public void init() {
        ...
        try {
            ...
            /*String war = getDocumentBase().getPath();

            System.out.println("getDocumentBase().getPath() = " + war);

            if (war.length() > 0) {
                if (war.indexOf("/", 1) > 0) {
                    war = war.substring(0, war.indexOf("/", 1));
                    war += "/";
                } else
                    war = "/";
            }*/
            ...
            String MainURL = getParameter("URL");

            System.out.println("Server.MAIN_URL = " + MainURL);

            mSSPURL = MainURL + "SSPServer";

            mServletURL = new URL(MainURL + "lmscmi");


            System.out.println("servlet url is " + mServletURL.toString() + ", mSSPURL = " + mSSPURL);
    ...
    public String Initialize(String iParam) {
        ...
        try{
            ...
            //ۤДобавила проверку, чтобы не было NullPounterException
            if (iParam == null) {
                iParam = "";
            }
    ...
    public String Terminate(String iParam) {
        ...
        try{
            ...
            //Добавила проверку чтобы не появился NullPointerException
            if (iParam == null) {
                iParam = "";
            }
            ...
            if (!(tempParm == null || tempParm.equals(""))) {
                ...
                if (!result.equals(STRING_TRUE)) {
                    ...
                    if (exitValue.equals("time-out")) {
                        ...
                        if (event != null) { // SeqNavRequests.NAV_CONTINUE
                            if (event.equals("3")) {
                                tempEvent = "next";
                            }
    ...
    public String GetValue(String iDataModelElement) {
        ...
        //Добавлено чтобы не вызвать NullPointerException
    ...
    public void setNumAttempts(String iNumAttempts) {
        if (iNumAttempts == null || iNumAttempts.equals("null")) {
            mNumAttempts = 0;
            return;
        }
        try {
