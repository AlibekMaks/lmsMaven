                              ...
                              HttpServletResponse oResponse) 
   {
      ...
      case ServletRequestTypes.NEW_OBJ:
         ...
         case ServletRequestTypes.IMPORT_COURSE:
            ...
            courseService = new CourseService();
             courseService.importCourse(iRequest, true);
      ...
      case ServletRequestTypes.IMPORT_MULTIPLE_COURSES:
            ...
            {           
                ...
                if (uploadFiles.length == 0) 
                {
                     ...
                     mDspImportStatus = "/import/dsp_invalidMultipleImport.jsp";  
                ...
                {   
                     ...
                     if ( filename.length() > 4 ) 
                     {
                         ...
                         {  
                             ...
                             resultsVector.add(mValidationResult);
                         }
                         mDspImportStatus = "/import/dsp_confirmMultipleImport.jsp";
                     }
                }
            }
            if ( notADirectory) 
            ...
            {    for (int j = 0; j < resultsVector.size(); j++ ) 
                {
                     ValidationResults lValidationResults = new ValidationResults();
                     lValidationResults = (ValidationResults)resultsVector.elementAt(j);
                     
                     if (lValidationResults.getRedirectView() == "/import/dsp_invalidImport.jsp" ) 
                     {
                         mDspImportStatus = "/import/dsp_invalidMultipleImport.jsp";
                         someFailed = "true";
                         
                         mValidationResult = lValidationResults;
                         failedNames.add(coursenames.elementAt(j)); 
                         validationErrors.add(lValidationResults);  
                     }
                 }
                 
            }
            
            iRequest.setAttribute("coursenames", failedNames);
            iRequest.setAttribute("validationFailures", validationErrors);
            iRequest.setAttribute("someFailed", someFailed);
            iRequest.setAttribute("result", mValidationResult);
            launchView(mDspImportStatus, iRequest, oResponse);
